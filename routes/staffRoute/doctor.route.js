...
router.get('/', async (req, res) => {
  ...
  res.render('vwDoctor/home', { appts, today });
});
...
router.get('/pet/:id/history', async (req, res) => {
  ...
  res.render('vwDoctor/pet-history', {
    history,
    packages
  });
});
...
router.get('/medicine', async (req, res) => {
  ...
  res.render('vwDoctor/medicine', { list });
});

/* ================== FORM TẠO BỆNH ÁN ================== */
router.get('/exam', (req, res) => {
  res.render('vwDoctor/exam', {
...
router.post('/exam', async (req, res) => {
  ...
  res.redirect(`/exam/${exam.examination_id}/prescribe`);
});

/* ================== FORM KÊ TOA ================== */
router.get('/exam/:exam_id/prescribe', async (req, res) => {
  const meds = await medicineModel.searchMedicine();
  res.render('vwDoctor/prescribe', { meds, exam_id: req.params.exam_id });
});

/* ================== KÊ TOA ================== */
router.post('/exam/:exam_id/prescribe', async (req, res) => {
...
router.get('/vaccination', async (req, res) => {
  ...
  res.render('vwDoctor/vaccination', { pets, packages, pet_id, petProfile });
});

// 2) Chọn 1 gói → hiện danh sách vaccine (mũi) trong gói
router.get('/vaccination/package/:package_id', async (req, res) => {
  const package_id = req.params.package_id;
  const pet_id = req.query.pet_id;

  const dueVaccines = await vaccModel.getDueVaccines(package_id);

  res.render('vwDoctor/vaccination-detail', {
    package_id,
    pet_id,
    dueVaccines
  });
...
router.post('/vaccination/confirm', async (req, res) => {
  ...
  try {
    ...
    res.redirect(`/vaccination/package/${package_id}?pet_id=${pet_id}`);
